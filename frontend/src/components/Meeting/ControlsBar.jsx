...
const ControlButton = ({ icon, label, onClick, active = false, danger = false, disabled = false, badge = null, title: titleProp }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    ...
    style={{
      ...
      gap: 2, padding: '8px 6px', borderRadius: 10,
      ...
      color: (danger || active) ? '#fff' : disabled ? 'var(--text-muted)' : 'var(--text-secondary)',
  ...
  >
    ...
    <span style={{ fontSize: '0.6rem', fontFamily: 'Syne, sans-serif', fontWeight: 600, whiteSpace: 'nowrap' }}>
      {label}
    </span>
    {badge && (
      <div style={{
...
export default function ControlsBar({
  ...
  activePanel, networkQuality, participantCount, screenShareSupported = true,
  ...
  const handleShareClick = () => {
    if (!screenShareSupported && !isScreenSharing) {
      import('react-hot-toast').then(({ toast }) => {
        toast.error('Screen sharing is not supported on mobile browsers. Use desktop Chrome, Edge or Firefox.', { duration: 5000 });
      });
      return;
    }
    onToggleScreenShare();
  ...
  return (
    <div style={{
      ...
      padding: isMobile ? '8px 4px' : '8px 12px', flexShrink: 0, position: 'relative',
    }}>
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        gap: isMobile ? 3 : 6, flexWrap: 'wrap',
      }}>
        ...
        <ControlButton
          icon={isVideoOff ? '📷' : '📹'}
          label={isVideoOff ? 'Start vid' : 'Stop vid'}
        ...
        <ControlButton
          ...
          title={!screenShareSupported ? 'Screen sharing not supported on mobile' : (isScreenSharing ? 'Stop sharing' : 'Share screen')}
        />
        ...
        <div style={{ position: 'relative' }}>
          ...
          {showMore && (
            <div className="animate-scaleIn" style={{
              ...
              background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12,
              padding: 6, marginBottom: 6, minWidth: 190, zIndex: 50,
            }}>
              <button onClick={() => {
                navigator.clipboard.writeText(`${window.location.origin}/join/${meetingId}`);
                setShowMore(false);
                import('react-hot-toast').then(({ toast }) => toast.success('Link copied!'));
              }} style={{
                background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left',
                padding: '10px 14px', borderRadius: 8, fontSize: '0.8125rem',
                color: 'var(--text-primary)', fontFamily: 'DM Sans, sans-serif', width: '100%',
              }}>
                🔗 Copy meeting link
              </button>
              <button onClick={() => {
                ...
                setShowMore(false);
                import('react-hot-toast').then(({ toast }) => toast.success('ID copied!'));
              }} style={{
                background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left',
                padding: '10px 14px', borderRadius: 8, fontSize: '0.8125rem',
                color: 'var(--text-primary)', fontFamily: 'DM Sans, sans-serif', width: '100%',
              ...
              {!screenShareSupported && (
                <div style={{
                  padding: '8px 14px', fontSize: '0.72rem', color: 'var(--warning)',
                  borderTop: '1px solid var(--border)', marginTop: 4, lineHeight: 1.4,
                }}>
                  📵 Screen sharing only works on desktop browsers (Chrome, Edge, Firefox)
                </div>
              )}
            </div>
          )}
      ...
      <div style={{
        ...
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{
            ...
            background: (networkQuality?.uplink || 0) === 0 || (networkQuality?.uplink || 0) <= i
